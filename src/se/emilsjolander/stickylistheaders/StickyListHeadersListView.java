 ...
 * This is because of 2 reasons: 1. It acts like as ListView. 2. It used to be a
 * ListView subclass and refactoring the name would cause compatibility errors.
 * 
...
public class StickyListHeadersListView extends FrameLayout {
		...
		if (attrs != null) {
			...
			try {
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
					mList.setOverScrollMode(a
							.getInt(R.styleable.StickyListHeadersListView_android_overScrollMode,
									0));
				}
				...
				mList.setTranscriptMode(a
						.getInt(R.styleable.StickyListHeadersListView_android_transcriptMode,
	...
	private void updateHeader(int headerPosition) {
			...
			if (mHeaderId == null || mHeaderId != headerId) {
				...
				if (mOnStickyHeaderChangedListener != null) {
					mOnStickyHeaderChangedListener.onStickyHeaderChanged(this,
							mHeader, headerPosition, mHeaderId);
				}
		...
		for (int i = 0; i < mList.getChildCount(); i++) {
			...
			if (child.getTop() >= stickyHeaderTop()
					&& (doesChildHaveHeader || isChildFooter)) {
				headerOffset = Math.min(
						child.getTop() - mHeader.getMeasuredHeight(),
						headerOffset);
				break;
			}
		}
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		if (action == MotionEvent.ACTION_DOWN) {
			mDownY = ev.getY();
			mHeaderOwnsTouch = mHeader != null
					&& mDownY <= mHeader.getHeight() + mHeaderOffset;
		}
		...
		if (mHeaderOwnsTouch) {
			if (mHeader != null && Math.abs(mDownY - ev.getY()) <= mTouchSlop) {
				...
				MotionEvent downEvent = MotionEvent.obtain(ev.getDownTime(),
						ev.getEventTime(), ev.getAction(), ev.getX(), mDownY,
						ev.getMetaState());
	...
	public int getHeaderOverlap(int position) {
		boolean isStartOfSection = isStartOfSection(Math.max(0, position
				- getHeaderViewsCount()));
		if (!isStartOfSection) {
